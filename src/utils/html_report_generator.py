...
class HTMLReportGenerator:
    ...
    def _create_heatmap(self, results: List[Dict[str, Any]]) -> go.Figure:
        ...
        criteria_display_names = [
            _truncate_label(name, max_length=24 if len(criteria_names) > 6 else 18)
        ...
        fig.update_layout(
            ...
            yaxis={"tickfont": {"color": "#6b6b6b", "size": 11}, "automargin": True},
        )
    ...
    def _create_radar_chart(self, results: List[Dict[str, Any]], top_n: int = 3) -> go.Figure:
        ...
        fig.update_layout(
            polar={
                "radialaxis": {"visible": True, "range": [0, 10], "gridcolor": "#2a2a2a"},
                "angularaxis": {
                    "rotation": 90,
                    "direction": "clockwise",
    ...
    def _get_executive_summary(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_score = sum(r.get("overall_score", 0) for r in successful_results) / len(
            successful_results
        )
        ...
        return {
            ...
            "avg_score": avg_score,
            "leader": {"name": leader.get("site_name"), "score": leader.get("overall_score")},
            "weakest": {"name": weakest.get("site_name"), "score": weakest.get("overall_score")},
        }
    ...
    def _get_strategic_insights(self, results: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        avg_by_criterion = {
            name: sum(scores) / len(scores) for name, scores in criteria_scores.items()
        }

        # Find weakest criterion (opportunity)
        weakest_criterion = (
            min(avg_by_criterion, key=avg_by_criterion.get) if avg_by_criterion else "N/A"
        )
        weakest_avg = avg_by_criterion.get(weakest_criterion, 0)

        # Find strongest criterion (threat)
        strongest_criterion = (
            max(avg_by_criterion, key=avg_by_criterion.get) if avg_by_criterion else "N/A"
        )
    ...
    def _get_evidence_data(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, comp in enumerate(sorted_comps):
            ...
            # Get top strength
            strengths = [c for c in comp.get("criteria_scores", []) if c["score"] >= 8]
            if strengths:
            ...
            for state in notable_states[:2]:
                ...
                if any(
                    word in state.lower() for word in ["good", "excellent", "clear", "prominent"]
                ):
                    evidence_list.append(
                        {"icon": "check-circle-2", "icon_class": "success", "text": state[:100]}
                    )
                elif any(
                    word in state.lower() for word in ["missing", "lack", "unclear", "hidden"]
                ):
                    evidence_list.append(
                        {"icon": "alert-circle", "icon_class": "warning", "text": state[:100]}
                    )
                else:
                    evidence_list.append(
                        {"icon": "info", "icon_class": "info", "text": state[:100]}
                    )

            # Get a weakness if available
            weaknesses = [c for c in comp.get("criteria_scores", []) if c["score"] < 6]
        ...
        for result in competitor_results:
            ...
            try:
                ...
                if isinstance(notable_states, list):
                    result["notable_states"] = [
                        str(state).strip() for state in notable_states if str(state).strip()
                    ]
    ...
    def generate_index_page(self, reports: List[Dict[str, Any]]) -> str:
        """
        ...
        html_content = template.render(
            reports=reports, timestamp=datetime.now().strftime("%B %d, %Y")
        )
...
"""
    ...
    def generate_html_report(
        ...
        successful_competitors = [
            r for r in results if r.get("success") and r.get("overall_score") is not None
        ]
