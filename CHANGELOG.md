...
### Added
- **🤖 GLM Analyzer**: Alternative LLM provider support via OpenAI-compatible API
  - New `GLMUXAnalyzer` class in `src/analyzers/glm_analyzer.py`
  - Supports GLM-4-flash via ZhipuAI's OpenAI-compatible endpoint
  - Same two-pass pipeline as ClaudeUXAnalyzer (observation + scoring)
  - Drop-in compatible interface for testing alternative models
- **📦 OpenAI Dependency**: Added `openai>=1.0.0` to requirements.txt for GLM support

### Documentation
- Updated README.md with GLM provider configuration options
- Added environment variable documentation for `LLM_PROVIDER` and `GLM_API_KEY`
